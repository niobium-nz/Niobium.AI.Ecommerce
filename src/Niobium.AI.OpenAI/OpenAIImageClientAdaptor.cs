...
namespace Niobium.AI.OpenAI
{
    internal class OpenAIImageClientAdaptor(IImageGenerator imageGenerator) : IImageClient
    {
        ...
        public async Task<IEnumerable<BinaryData>> RunAsync(string prompt, int width, int height, int variantCount = 1, List<ImageReference>? references = null, CancellationToken cancellationToken = default)
        {
            ...
            else
            {
                ...
                images = await imageGenerator.EditImageAsync(
                    new DataContent(reference.Data, reference.Data.MediaType ?? "image/png"),
                    prompt,
                    options: options,
                    cancellationToken: cancellationToken);
            }
