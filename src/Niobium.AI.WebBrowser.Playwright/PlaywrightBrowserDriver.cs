...
namespace Niobium.AI.WebBrowser.Playwright
{
    internal sealed class PlaywrightBrowserDriver(IOptions<PlaywrightOptions> options, IPlaywright playwright) : IWebBrowser
    {
        ...
        public async Task WaitForAsync(int tabId, WebBrowserWaitForRequest waitCondition, CancellationToken cancellationToken = default)
        {
            ...
            switch (waitCondition.Target)
            {
                ...
                case WebBrowserWaitTarget.LoadState:
                    await page.WaitForLoadStateAsync(MapLoadState(waitCondition.LoadState ?? WebBrowserLoadState.Load), new PageWaitForLoadStateOptions
                    {
                        Timeout = waitCondition.TimeoutMs,
                    }).ConfigureAwait(false);
                    break;
        ...
        private static WaitForSelectorState? MapSelectorState(WebBrowserWaitForSelectorState? state)
            => state switch
            {
                WebBrowserWaitForSelectorState.Attached => WaitForSelectorState.Attached,
                WebBrowserWaitForSelectorState.Detached => WaitForSelectorState.Detached,
                WebBrowserWaitForSelectorState.Visible => WaitForSelectorState.Visible,
                WebBrowserWaitForSelectorState.Hidden => WaitForSelectorState.Hidden,
                _ => null
            };

        private static LoadState MapLoadState(WebBrowserLoadState state)
            => state switch
            {
                WebBrowserLoadState.Load => LoadState.Load,
                WebBrowserLoadState.DOMContentLoaded => LoadState.DOMContentLoaded,
                WebBrowserLoadState.NetworkIdle => LoadState.NetworkIdle,
                _ => LoadState.Load
            };
