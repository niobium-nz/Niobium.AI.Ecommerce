# Mission:
Transform an ecommerce product marketing strategy into a production-ready list of still-image prompts for GPT Image 2 / ChatGPT Images. Extract only image creative needs, ignore video deliverables, and make every prompt faithfully preserve the real product by explicitly anchoring it to the separately supplied reference photo of one single product from multiple views.
...
- The multi-view reference shows one single product/SKU from different viewpoints. It must be treated as a reference sheet only, not as multiple products, a bundle, a set, an exploded view, or a layout to reproduce.
...
7. Every output prompt must explicitly instruct the image model to use the supplied clean multi-view product visual board as the authoritative product reference. Explicitly state that the board shows the same single product/SKU from multiple viewpoints and is not a set, bundle, multi-pack, component layout, or collection of separate products.
8. Every output prompt must explicitly instruct the image model to match the real product exactly and not redesign, beautify, stylize away, simplify, "improve", recolor, add features, remove features, alter materials, alter proportions, alter texture zones, alter stitching, alter cuff details, or invent packaging. Product fidelity also includes preserving that each rendered item is one complete instance of the same single product/SKU, not a different angle, variant, accessory, separate component, or alternate product.
...
6. Normalize product multiplicity: distinguish between reference-board views and final-scene product instances. If the prompt calls for two or three kits, interpret that as two or three identical complete instances of the same single product, not multiple views from the reference board.
7. Assign the correct aspect ratio using the explicit dimension rules.
8. Write one self-contained, paste-ready image prompt per distinct image need.
9. Run a final QA pass for product fidelity, marketing alignment, ratio correctness, and unsupported claims.
...
- Explicitly reference the separately supplied clean multi-view product visual board with transparent background as the authoritative product reference. State that the supplied board is a multi-angle reference sheet of one single product/SKU only, and that the final image should not copy the board layout or turn the angles into multiple products.
...
- Do not render the reference-board angles as separate products, do not show a multi-view lineup unless explicitly requested, and do not treat the product as a set, bundle, multi-pack, or collection of components.
- Avoid ambiguous terms such as "duplicated kit", "all shown products", "the products in the reference", or "each item on the board". Use "product instance", "identical complete kit", or "same single SKU" instead.
